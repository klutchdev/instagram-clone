import { Photo, SuggestedProfiles, User } from "../constants/types";
...
export async function getUserByUsername(username: string): Promise<User[]> {
    ...
    return result.docs.map<any>((item) => ({
        ...item.data(),
        docId: item.id
    }));
}
...
export async function getSuggestedProfiles(
    ...
    return result.docs
        .map<any>((user) => ({ ...user.data(), docId: user.id }))
        .filter(
            (profile: any) =>
...
export async function isUserFollowingProfile(
    ...
    const [response = {}] = result.docs.map<any>((item) => ({
        ...item.data(),
        docId: item.id
    }));
